...
現在は実際にCloudflare上で動かしながら改善しています。開発はオーナーが仕様を考え、Codexの実装支援を受けながら進めています。
...
- サブスク登録と毎月の固定費集計
- バイト先ごとの締め日 / 給料日登録
- バイト予定からの深夜料金・休憩時間込みの概算給料計算と給料日表示
- バイトごとの給料見込み表示
...
サブスク設定では、名前、月額、請求日、支払いカード、カテゴリ、メモを登録できます。

- 請求日は `1` のような日付、または `末` を指定できます。
- 支払いカードを選ぶと、クレカの締め日 / 支払日から実際の引き落とし日に反映されます。
- カード未選択の場合は、請求日をそのまま支出日として扱います。
- 有効なサブスクは毎月の支出として自動集計されます。
- カレンダーのお金表示、お金モード、日次/月次の振り返りに反映されます。
- 通常の支出記録とは別データとして保存するため、金額や支払日を変えると以後の月次計算に反映されます。

## Payroll Calculation

バイト先設定では、時給、深夜時給、締め日、給料日を登録できます。

- 既存のバイト先は未設定項目を `末締め / 25日払い` として扱います。
- 締め日と給料日は `15` のような日付、または `末` を指定できます。
- バイトタグが付いた予定の開始時刻 / 終了時刻から勤務時間を計算します。
- 22:00から翌5:00までを深夜時間として扱います。
- 深夜時給が未設定の場合は、通常時給の25%増しで計算します。
- 休憩時間は無給として給料計算から差し引きます。
- 予定ごとに休憩時間を入力できます。未入力なら勤務時間から自動推定します。
- 自動推定は `6時間以上:30分 / 8時間超:60分 / 6時間未満:0分` です。
- 勤務日は次に来る締め日に集計し、その締め日の直後に来る給料日に支払われる想定です。
- 例: `15日締め / 25日払い` なら、15日までの勤務は同月25日の給料に入ります。
- 給料日はカレンダー上と予定一覧に表示されます。

これはフロント側の計算ロジックなので、Cloudflareから自宅サーバーへ移行しても同じデータで動かせます。
